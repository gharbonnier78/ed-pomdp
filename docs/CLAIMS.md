...
| CLM-IDENT-001 | System state S and evidence quality E are not identifiable from arbitrary repeated observations; separation requires structural assumptions such as heterogeneous channels or controlled interventions. | Formal claim | Counterexample established; empirical status open | FORMAL | Refute the non-identifiability case by distinguishing aliased latent pairs from the unchanged channel alone. | `identifiability/identifiability_note.tex` | Synthetic identifiable / weak / non-identifiable regimes |
